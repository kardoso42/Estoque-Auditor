...
export function detectarMapeamento(headers){
  const regras={codigo:['codigo','codigoproduto','codigodoproduto','idproduto','id'],sku:['sku','codigosku','codigodasku'],gtin:['gtin','ean','codigoebar','codigobarras','codigodebarras','codigoeangt','codigoeangt'],nome:['produto','nome','nomeproduto','descricao','descricaoproduto','descricaodoproduto'],familia:['familia','familiadeproduto','familiadoproduto','grupo','categoria'],local:['local','localestoque','localdeestoque','localizacao','localizacaodeestoque','deposito','almoxarifado'],quantidade:['quantidade','qtd','qtdestoque','estoque','estoqueatual','saldo','saldodisponivel','quantidadedeestoque'],unidade:['unidade','und'],custo:['custo','custounitario','valorcusto','cmcunitario','cmctotal']};
  const result={}; for(const campo of CAMPOS){const aliases=regras[campo]||[];result[campo]=headers.find(h=>aliases.includes(normalizar(h)))||''} return result;
}

export function detectarLinhaCabecalho(rows,maxRows=20){let melhor={index:0,headers:[],score:0};for(let i=0;i<Math.min(rows.length,maxRows);i++){const headers=Object.keys(rows[i]||{});const mapping=detectarMapeamento(headers);const score=CAMPOS.filter(c=>mapping[c]).length;if(score>melhor.score)melhor={index:i,headers,score}}return melhor}

function numero(valor){if(typeof valor==='number')return Number.isFinite(valor)?valor:0;let s=String(valor??'').trim();if(!s)return 0;s=s.replace(/\s/g,'');if(s.includes(',')&&s.includes('.'))s=s.lastIndexOf(',')>s.lastIndexOf('.')?s.replace(/\./g,'').replace(',','.'):s.replace(/,/g,'');else if(s.includes(','))s=s.replace(',','.');return Number(s)||0}
